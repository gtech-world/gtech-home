      ...
      {data.length &&
        data.map((v: any, i: number) => {
          return v.render ? (
            ...
            <Link href={v.href} className="link-hover" key={`${i}`}>
              {v.name}
            </Link>
          ) : (
            <span>{v.name}</span>
  ...
  const navList = [
    ...
    },
    {
      ...
      children: [
        {
          render: () => {
            return <img className="w-[7.5rem] mt-2" src="/images/wechat_qrcode.png" alt="" />;
          },
        },
      ],
    },
    ...
    <div className="pb-5 bg-bgc md:px-8 md:py-12">
      <div className="flex flex-col mx-auto w-container md:w-full">
          ...
          <div className="flex flex-col justify-between">
            ...
            <ul className="flex mt-5 md:flex-col">
              <li className="mr-14 mt-[-1.3rem]">
                <Link className="md:float-left" href="/">
              ...
              {navList.map((v, i) => {
                return (
                  <li
                    key={`${i}`}
                    className={classNames("md:mt-5", i === navList.length - 1 ? "mr-0" : "mr-[3.75rem] md:mr-2")}>
                    {v.href && !v.children ? (
                      <Link className="text-xl font-bold link-hover md:text-xl" href={v.href}>
                        {v.name}
                      </Link>
                    ) : (
        ...
        <div className="flex justify-between pt-4 mt-16 text-sm border-t border-black md:flex-col md:mt-8">
          ...
          <span
            onClick={() => window.open(i18n.language === "zh" ? "/zhstatement" : "/enstatement", "_blank")}
            className="cursor-pointer md:mt-5 link-hover">
            {t("footer.viewBtn")}
          </span>
        </div>
      </div>
