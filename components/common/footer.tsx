  ...
  return (
    <ul
      className={classNames(
        "flex flex-col text-base mt-4 leading-[2.2] font-normal md:text-[0.9375rem]",
        className
      )}
    >
      {data.length &&
      ...
      <div className="flex flex-col mx-auto w-container md:w-full">
              ...
              {navList.map((v, i) => {
                return (
                  <li
                    ...
                    className={classNames(
                      "md:mt-5",
                      i === navList.length - 1 ? "mr-0" : "mr-[3.75rem] md:mr-2"
                    )}
                  >
                    {v.href && !v.children ? (
                      <Link
                        className="text-xl font-bold link-hover md:text-xl"
                        href={v.href}
                      >
                        {v.name}
                      </Link>
        ...
        <div className="flex justify-between pt-4 mt-16 text-sm border-t border-black md:flex-col md:mt-8">
          ...
          <span
            onClick={() =>
              window.open(
                i18n.language === "zh" ? "/zhstatement" : "/enstatement",
                "_blank"
              )
            }
