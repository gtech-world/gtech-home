...
export function Policy() {
  ...
  return (
    <div className={"fixed bottom-0 left-0 z-10 h-20 w-full z-10 flex justify-center items-center bg-white text-sm shadow-[0_-3px_15px_0_rgba(0,0,0,0.1)] md:flex-col md:px-5 md:h-auto md:pb-5"}>
      ...
      <div onClick={()=>{setCookie('statement','access');setPolicyVisible(false)}} className="border border-black bg-black text-white px-4 h-11 rounded flex items-center justify-center ml-5 cursor-pointer md:w-full md:mb-4 md:ml-0">{t('footer.acceptBtn')}</div>
      ...
      <FiX onClick={()=>setPolicyVisible(false)} className="absolute right-8 text-xl cursor-pointer md:top-4 md:right-5" />
    </div>
  );
}
