...
export function Policy(p:any) {
  ...
  return (
    <div className={classNames("fixed bottom-0 left-0 z-10 h-20 w-full z-10 flex justify-center items-center bg-white text-sm shadow-[0_-3px_15px_0_rgba(0,0,0,0.1)] md:flex-col md:px-5 md:h-auto md:pb-5",className)}>
      <span className="inline-block md:px-8 md:text-center md:pb-8 md:pt-12">{t('footer.text')}</span>
      <div onClick={onAccess} className="flex items-center justify-center px-4 ml-5 text-white bg-black border border-black rounded cursor-pointer h-11 md:w-full md:mb-4 md:ml-0">{t('footer.acceptBtn')}</div>
