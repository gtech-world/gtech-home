...
export function Pagination(p: Props & HTMLAttributes<HTMLDivElement>) {
  ...
  const items: Item[] = useMemo(() => {
    ...
    if (pgNum >= count - 2)
      return [{ name: "1" }, { name: omit }].concat(_.range(pgNum - 2, count).map((i) => ({ name: `${i + 1}` })));
    return [
      { name: "1" },
      { name: omit },
      { name: pgNum - 1 + "" },
    ...
    <div className={classnames("flex justify-center text-[0.8125rem] text-center leading-8 font-semibold", className)}>
      <div className="flex">
        {count > 1 && (
          <span
            onClick={doFirst}
            className="mx-[0.185rem] flex justify-center items-center cursor-pointer border min-w-[32px] h-[32px] rounded-lg">
        ...
        {items.map((page, i) => (
          <div
            ...
            className={classnames(
              ...
              "mx-[0.185rem] flex justify-center items-center min-w-[32px] h-[32px] px-1.5 rounded-lg",
        ...
        {count > 1 && (
          <span
            onClick={doNext}
            className="mx-[0.185rem] rotate-180 flex justify-center items-center cursor-pointer border min-w-[32px] h-[32px] rounded-lg">
            <img src="/images/arrow_page.svg" alt="" />
          </span>
        )}
        {count > 1 && (
