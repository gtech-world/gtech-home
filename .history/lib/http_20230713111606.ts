...
export async function getNewsListTotal(newsTypeId: number=1, pageNumber: number=1,pageSize:number=10) {
  const res = await axios.get(creatUrl(`/wechat/news?newsTypeId=${newsTypeId}&pageNumber=${pageNumber}&pageSize=${pageSize}`));
  return getData(res);
}
export async function getNewsCount (newsTypeId:number = 1,newsType:string){
  const res = await axios.get(creatUrl(`/wechat/news/count`));
  return getData(res);
}
