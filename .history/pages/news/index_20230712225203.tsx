...
export default function Index() {
  ...
  return (
    <HeaderLayout headerProps={headerProps}>
      <div className="flex mt-10 w-[75rem] md:w-[21.9375rem]  mx-[155px] md:mx-5 rounded-lg shadow-lg justify-between">
        {newsType.map((e) => {
          return (
            <div
              key={`type_${e.id}`}
              className="w-[360px] h-[12.75rem] md:w-[10.625rem] rounded-lg shadow-lg  md:mr-[1.25rem] "
            >
              ...
              <div className='flex flex-row flex-wrap'>
                {e.children.map((item: NewsTypesController.ListRecord) => {
                  return (
                    <div
                      className=' bg-[#E9E9E9] min-w-[20px] h-[38px] flex items-stretch ml-5 mt-5 rounded-[0.25rem] shadow-lg px-[1rem]'
                    >
                    {item.typeName}
                    </div>
                  );
