...
export default function Index() {
  ...
  return (
    <HeaderLayout headerProps={headerProps}>
      <div className="flex  w-[75rem] bg-red-100 md:w-[21.9375rem] mx-[150px] md:mx-5 rounded-lg shadow-lg">
        {newsType.map((e) => {
          return (
            <div
              key={`type_${e.id}`}
              className="w-[360px] h-[12.75rem] md:w-[10.625rem] rounded-lg shadow-lg mr-[3.75rem] md:mr-[1.25rem] "
            >
              ...
              <div className='flex flex-row flex-wrap'>
                {e.children.map((item: NewsTypesController.ListRecord) => {
                  return (
                    <div
                      className=' bg-[#E9E9E9] min-w-[20px] ml-5 mt-5 rounded-[0.25rem] shadow-lg'
                    >
                      <div className="px-[1.25rem] ">{item.typeName}</div>
                    </div>
                  );
                })}
              </div>
