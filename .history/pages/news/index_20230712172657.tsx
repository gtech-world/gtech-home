          ...
          return <><div key={`type${e.id}`} className="w-[22.5rem] h-[12.75rem] md:w-[10.625rem] rounded-lg shadow-lg mr-[3.75rem] md:mr-[1.25rem] ">
            ...
            <div className="flex w-[20rem]">
              {e.children.map((item: NewsTypesController.ListRecord) => {
                ...
                return <div style={{marginLeft:'20px',marginRight:'20px'}} className={`flex wrap  items-center text-[1rem] w-[${countStrWidth(item.typeName)}rem] h-[2.375rem] bg-[#E9E9E9] mt-[1.25rem] ml-[1.25rem]`}>{item.typeName}</div>;
              })}
            </div>
          </div>
           </>
