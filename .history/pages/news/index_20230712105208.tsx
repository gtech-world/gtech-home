    ...
    <div className="mt-16 w-container md:w-full md:mt-8 md:px-3">
      {
        !!data.length && data.map((v,i)=>{
          ...
          return(
            <div key={`data${i}`} className="flex mb-12 md:items-center md:mb-5">
...
export default function Index() {
  ...
  return(
    <HeaderLayout headerProps={headerProps}>
      <div>
        <div className='max-w-sm  ml-[9.5rem] mt-10 mx-10 bg-white shadow-lg h-[12.75rem] w-[22.5rem] rounded-xl  sm:space-y-0 sm:space-x-6'>
          <div className="w-full h-[3.75rem] p-4 bg-green-1 shadow-lg  text-white">数字双碳</div>
          <div className="flex w-full  mr-[9.5rem] bg-slate-600">
          <div className="w-32 px-2 ml-10 bg-white shadow-lg">数字碳知识库</div>
          <div className="w-32 px-2 ml-10 bg-white shadow-lg">数字碳知识库</div>
          </div>
