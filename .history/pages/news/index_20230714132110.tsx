      ...
      {!!data.length &&
        data.map((v, i) => {
          ...
          return (
            ...
            <div
              key={`data${i}`}
              className="flex mb-12   md:items-center md:mb-5 w-full  h-[200px]"
            >
              <div
              ...
              <div className="flex flex-col justify-between">
                <div className="">
                  <h4 className="text-2xl font-semibold md:text-base">
                    <Link
                      className="ellipsis-2 text-[20px]"
                      rel="opener"
                  ...
                  <time className="inline-block md:py-0 text-[14px]  text-gray-2">
                    {moment(v.newsUpdateTime * 1000).format(
                      "YYYY-MM-DD HH:mm:ss"
                    )}
                  </time>
                  <p className="md:hidden text-[14px]">{v.digest}</p>
                  <div className="flex flex-row mt-[10px]" >{v?.newsTypes.map((e)=>{
...
export default function Index() {
  ...
  const [selected, setSelected] = useState<Partial<NewsTypesController.TypeList>>( newsType[0]);
  ...
  const getListTotal = async () => {
   const isExist= JSON.stringify(selected) === '{}' ?  newsType[0] :selected
    const res = await getNewsListCount(isExist?.typeName, isExist?.id)
  ...
  const getList =async () =>{
   const isExist= JSON.stringify(selected) === '{}' ?  newsType[0] :selected
    const res = await getNewsList( isExist.id, pgNum, pgSize)
    setData(res || [])
  ...
  return (
    <HeaderLayout headerProps={headerProps}>
      ...
      <div className="flex flex-col  mx-[155px] md:mx-0 ">
              ...
              `}>
                ...
                <div className='flex flex-row flex-wrap'>
                  {e.children.map((item: NewsTypesController.ListRecord) => {
                    console.log('dasdasxcz',selected.typeName === item.typeName );
                    
                    return (
                      <div
                      onClick={()=>setSelected(item)}
                        className={` ${selected.typeName === item.typeName && 'text-[#29953A]  bg-[#29953A1A] '} cursor-pointer  bg-[#E9E9E9] min-w-[20px] h-[38px] flex items-center ml-5 mt-5 rounded-[0.25rem] px-[1rem]`}
                      >
                        {item.typeName}
                      </div>
                    );
        ...
        <div className="mt-5 ">
        <div className="h-[26px] ">发布时间</div>


        {
          loading?<Loading className="h-[22rem] " />:<ArticleList cateId={JSON.stringify(selected) === '{}' ?  newsType[0] :selected} data={data} />
        }
