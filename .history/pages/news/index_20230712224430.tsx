...
export default function Index() {
  ...
  return (
    <HeaderLayout headerProps={headerProps}>
      <div className="flex  w-[75rem] md:w-[21.9375rem] mx-[150px] md:mx-5 rounded-lg shadow-lg justify-between">
        {newsType.map((e) => {
          return (
            <div
              key={`type_${e.id}`}
            ...
            >
              ...
              <div className='flex flex-row flex-wrap'>
                {e.children.map((item: NewsTypesController.ListRecord) => {
                  return (
                    <div
                      className=' bg-[#E9E9E9] min-w-[20px] ml-5 mt-5 rounded-[0.25rem] shadow-lg px-[1rem]'
                    >
                    {item.typeName}
                    </div>
                  );
