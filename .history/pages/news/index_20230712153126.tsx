...
export default function Index() {
  ...
  const mergedData = (arr:any[] = []) =>{
    console.log('arrrrsas',arr);
  ...
  const getNewsType = async() =>{
    try{
      ...
      const newArr = res.reduce((accumulator: { [x: string]: any[]; }, item: { typeGroup: string | number; }) => {
        if (!accumulator[item.typeGroup]) {
          accumulator[item.typeGroup] = [];
        }
        // accumulator[item.typeGroup].push(item);
        accumulator.children.push( item)
        return accumulator;
      }, {});
      setNewsType(newArr)
    }catch(e){
...
console.log('valuevalue',newsType);
  ...
  const { total, data } = useMemo(() => {
    ...
    console.log('value[1]value[1],value[1]',value[1],mergedData(value[1]));
  ...
  return (
    ...
    <div className="flex flex-wrap w-[75rem] md:w-[32.25rem] mx-40 md:mx-5 rounded-lg shadow-lg">
      <div className="w-[22.5rem] h-[12.75rem] md:w-[15.625rem] rounded-lg shadow-lg   ">
      ...
      <div className="w-[22.5rem] h-[12.75rem] md:w-[21.9375rem] rounded-lg shadow-lg">Box 3</div>
    </div>
